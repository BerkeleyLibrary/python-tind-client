...
def fetch_metadata(record: str, *, api_key: str, api_url: str) -> Record:
    ...
    :param str api_url: Base URL of the TIND instance.
...
def fetch_file(
    ...
    *,
    ...
    :param str api_key: The TIND API token.
    :param str output_dir: The output directory for this download.
    :param str default_storage_dir: Fallback output directory when output_dir is empty.
    ...
    output_target = output_dir or default_storage_dir
    (status, saved_to) = tind_download(
        file_url, output_dir=output_target, api_key=api_key
    )
...
def fetch_file_metadata(
    record: str, *, api_key: str, api_url: str
) -> list[dict[str, Any]]:
    ...
    status, files = tind_get(f"record/{record}/files", api_key=api_key, api_url=api_url)
...
def fetch_ids_search(query: str, *, api_key: str, api_url: str) -> list[str]:
    ...
    status, rec_ids = tind_get(
        "search", api_key=api_key, api_url=api_url, params={"p": query}
    )
...
def fetch_marc_by_ids(ids: list[str], *, api_key: str, api_url: str) -> list[Record]:
    ...
    records = []
    for item in ids:
        m = fetch_metadata(item, api_key=api_key, api_url=api_url)
        records.append(m)

    return records


def fetch_search_metadata(query: str, *, api_key: str, api_url: str) -> list[Record]:
    ...
    """
    ids = fetch_ids_search(query, api_key=api_key, api_url=api_url)

    return fetch_marc_by_ids(ids, api_key=api_key, api_url=api_url)


def _search_request(
    query: str, *, api_key: str, api_url: str, search_id: str | None = None
) -> str:
    ...
    if search_id:
        status, response = tind_get(
            ...
            api_key=api_key,
            api_url=api_url,
            params={"format": "xml", "p": query, "search_id": search_id},
        )
    else:
        status, response = tind_get(
            "search",
...
def _retrieve_xml_search_id(response: str) -> tuple[E.Element, str]:
...
def search(
    query: str, *, api_key: str, api_url: str, result_format: str = "xml"
) -> list[Any]:
    ...
    while True:
        if search_id:
            response = _search_request(
                query, api_key=api_key, api_url=api_url, search_id=search_id
            )
        else:
            response = _search_request(query, api_key=api_key, api_url=api_url)
